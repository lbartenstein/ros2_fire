...
def process_fire_detection(image, detection_order, yolo_models, model_lock):
    ...
    processed_image_resized = resized_image.copy()  # Copy the resized image
    if filtered_results_final:
        for res in filtered_results_final:
            ...
            if model_name == "Model 4":
                ...
                label = f"{yolo_models[model_name].names[class_id]} {confidence:.2f}"  
            x1, y1, x2, y2 = map(int, box)
            cv2.rectangle(processed_image_resized, (x1, y1), (x2, y2), (0, 255, 0), 2)  
